...
export default {
  ...
  enterFullScreen() {
    ...
    window.addEventListener('resize', this.setFullScreenHeight);
  },
  setFullScreenHeight() {
    if (!this.isFullScreen) return;
    const block = this.$refs.selectList;
  ...
  onItemSelected() {
    ...
    this.$refs.selectList.style.transform = 'translateY(0)';
    
    // Для iOS: повторный фокус после выбора элемента
    if (this.isMobile) {
      this.$nextTick(() => {
